...
export default function ArtistOrbMenu({ selection, onDismiss, onAction, detailsOpen: controlledDetailsOpen, onDetailsChange, expanded, expanding }: {
  ...
  useLayoutEffect(() => {
    ref.current?.querySelector<HTMLButtonElement>(detailsOpen ? ".detail-close" : "[data-avatar]")?.focus();
  }, [detailsOpen]);
  ...
  const panelY = Math.max(12, Math.min(cy - panelHeight / 2, viewport.height - panelHeight - 12));
  ...
  return (
    <div
      ...
      onWheel={onDismiss}
      onKeyDown={(event) => {
        if (event.key === "Escape") { event.stopPropagation(); if (detailsOpen) setDetailsOpen(false); else onDismiss(); }
        if (event.key !== "Tab") return;
        const items = Array.from(event.currentTarget.querySelectorAll<HTMLElement>("button:not(:disabled), a[href]"));
        const focusable = items.filter((item) => !item.closest("[inert]"));
          ...
          <div className="relative size-full" inert={detailsOpen} onWheel={(event) => event.stopPropagation()}>
            ...
            <div className="pointer-events-auto absolute left-1/2 top-full mt-3 flex w-56 -translate-x-1/2 flex-col items-center gap-2">
              <div className={`${MENU_ENTRANCE} ${TIMING.artist} flex max-w-full items-center gap-1 rounded-full bg-neutral-950/95 p-1 shadow-lg`}>
                ...
                <button type="button" aria-label={`Artist details for ${artist.name}`} className="group relative grid size-8 shrink-0 place-items-center rounded-full text-neutral-400 transition-colors hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-white" onClick={() => setDetailsOpen(true)}><InfoIcon size={16} /><span aria-hidden="true" className={`${tooltipClass} top-full mt-2`}>Artist details</span></button>
              </div>
            </div>
          </div>
        </foreignObject>
        {detailsOpen && <foreignObject x={panelX} y={panelY} width={panelWidth} height={panelHeight} className="overflow-visible">
          <div className="pointer-events-auto size-full overflow-hidden rounded-2xl border border-white/15 bg-neutral-900 shadow-2xl motion-safe:starting:opacity-0 motion-safe:starting:translate-y-1 transition-[opacity,translate] duration-200 motion-reduce:transition-none" onWheel={(event) => event.stopPropagation()}>
            <DetailPanel nodeId={artist.id} artist={artist} onClose={() => setDetailsOpen(false)} />
          </div>
        </foreignObject>}
      </svg>
    </div>
